...
void const *gpu_computeVertexAttributeDataPointer(
  ...
  size_t pointer = (size_t)head->buffer + head->offset + (size_t)gl_VertexID * head->stride;

  return (void const*)pointer;
}

void gpu_runVertexPuller(GPUVertexPullerOutput *const              output,
  ...
  while (i < MAX_ATTRIBUTES) {
	  const GPUVertexPullerHead *pointers = &puller->heads[i];
	  if (pointers->enabled == 0) {
		  output->attributes[i] = NULL;
	  } else {
		  output->attributes[i] = gpu_computeVertexAttributeDataPointer(pointers, index);
	  }
	  i++;
  }
}
 ...
 */
void gpu_runFrustumPlaneClippingOnEdge(float *const minT, float *const maxT,
...
void gpu_writeClippedTriangle_OneVertexVisible(
    ...
    size_t const visible, float const tMin[3], float const tMax[3],
  ...
  mix_Vec4(dstPositions + 1, srcPositions + vertex, srcPositions + nextVertex,
           nextT);

  mix_Vec4(dstPositions + 2, srcPositions + prevVertex, srcPositions + vertex,
           prevT);
...
void gpu_writeClippedTriangle_TwoVerticesVisible_FirstTriangle(
    ...
    Vec4 *const dstPositions, Vec4 const *const srcPositions,
    size_t const vertex, size_t const nextVertex, size_t const prevVertex,
    float const nextT, float const prevT) {
  ...
  mix_Vec4(dstPositions + 0, srcPositions + vertex, srcPositions + nextVertex,
           nextT);
...
void gpu_writeClippedTriangle_TwoVerticesVisible_SecondTriangle(
  ...
  mix_Vec4(dstPositions + 0, srcPositions + prevVertex, srcPositions + vertex,
           prevT);
...
void gpu_runFrustumPlaneClippingOnTriangle(
    GPUTriangle triangles[MAX_CLIPPED_TRIANGLES], size_t *const nofTriangles,
    GPUTriangle const *const triangle, FrustumPlane const plane) {
  assert(triangles != NULL);
  assert(nofTriangles != NULL);
  assert(triangle != NULL);
  ...
  if (visible == 0x1 || visible == 0x2 || visible == 0x4) {
    // fprintf(stderr,"one\n");
    gpu_writeClippedTriangle_OneVertexVisible(triangles, nofTriangles, visible,
                                              tMin, tMax, triangle);
    return;
  }
  ...
  if (visible == 0x3 || visible == 0x5 || visible == 0x6) {
    // fprintf(stderr,"two\n");
    gpu_writeClippedTriangle_TwoVerticesVisible(triangles, nofTriangles,
  ...
  if (visible == 0x7) {
    ...
    gpu_writeClippedTriangle_ThreeVerticesVisible(triangles, nofTriangles,
                                                  triangle);
    return;
  }
}
...
void gpu_runFrustumPlaneClippingOnTriangleList(
    GPUTriangleList *const output, GPUTriangleList const *const input,
  ...
  for (size_t i = 0; i < input->nofTriangles; ++i)
    gpu_runFrustumPlaneClippingOnTriangle(
        output->triangles, &output->nofTriangles, input->triangles + i, plane);
}
...
void gpu_runTriangleClipping(GPUTriangleList *const   output,
  ...
  // near plane clipping
  gpu_runFrustumPlaneClippingOnTriangle(output->triangles,
                                        &output->nofTriangles, input, NEAR);
}
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  /// \todo V této funkci spočtěte barycentrické coordináty trojúhelníku v
  /// obrazovce.
  ...
  //Pouzil som vzorec zo stranky: https://gamedev.stackexchange.com/questions/23743/whats-the-most-efficient-way-to-find-barycentric-coordinates

  float precalc = 1.0f / ((vertices[1].data[1] - vertices[2].data[1]) * (vertices[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0]) * (vertices[0].data[1] - vertices[2].data[1]));

  float lambda1 = ((vertices[1].data[1] - vertices[2].data[1]) * (pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[2].data[0] - vertices[1].data[0]) * (pixelCenter[0].data[1] - vertices[2].data[1])) * precalc;
  float lambda2 = ((vertices[2].data[1] - vertices[0].data[1]) * (pixelCenter[0].data[0] - vertices[2].data[0]) + (vertices[0].data[0] - vertices[2].data[0]) * (pixelCenter[0].data[1] - vertices[2].data[1])) * precalc;
  float lambda3 = 1.0f - lambda1 - lambda2;

  coords->data[0] = lambda1;
  coords->data[1] = lambda2;
  coords->data[2] = lambda3;
}
 ...
 */
void gpu_createFlatFragmentAttribute(GPUFragmentShaderInput *const fragment,
...
/**
 * @brief This function creates fragment attribute using noperspective interpolation
 *
 * @param fragment output fragment
 * @param primitive input primitive
 ...
 */
void gpu_createNoPerspectiveFragmentAttribute(
    GPUFragmentShaderInput *const fragment, GPUPrimitive const *const primitive,
    size_t attribute, size_t nofComponents, Vec3 const *const barycentrics) {
  for (size_t component = 0; component < nofComponents; ++component) {
    float const values[WEIGHTS_PER_BARYCENTRICS] = {
        ((float *)primitive->vertices[0].attributes[attribute])[component],
 ...
 */
void gpu_createSmoothFragmentAttribute(
    GPUFragmentShaderInput *const fragment, GPUPrimitive const *const primitive,
    size_t attribute, size_t nofComponents, Vec3 const *const barycentrics,
    float const homogeneousCoords[WEIGHTS_PER_BARYCENTRICS]) {
  for (size_t component = 0; component < nofComponents; ++component) {
    ...
    ((float *)fragment->attributes.attributes[attribute])[component] =
        gpu_smoothInterpolate(values, barycentrics->data, homogeneousCoords);
  }
}
 ...
 * @param homogeneousCoords weights of triangle vertices
 */
void gpu_createInterpolatedFragmentAttribute(
    GPUFragmentShaderInput *const fragment, GPUPrimitive const *const primitive,
    size_t attribute, Vec3 const *const barycentrics,
    float const homogeneousCoords[WEIGHTS_PER_BARYCENTRICS]) {
  size_t const nofComponents = (size_t)primitive->types[attribute];
  if (primitive->interpolations[attribute] == FLAT) {
    gpu_createFlatFragmentAttribute(fragment, primitive, attribute,
                                    nofComponents);
  } else if (primitive->interpolations[attribute] == NOPERSPECTIVE) {
    gpu_createNoPerspectiveFragmentAttribute(fragment, primitive, attribute,
                                             nofComponents, barycentrics);
  } else if (primitive->interpolations[attribute] == SMOOTH) {
    gpu_createSmoothFragmentAttribute(fragment, primitive, attribute,
                                      nofComponents, barycentrics,
                                      homogeneousCoords);
  }
}
...
void gpu_createFragment(GPUFragmentShaderInput *const fragment,
  ...
  for (size_t attribute = 0; attribute < MAX_ATTRIBUTES; ++attribute) {
    ...
    gpu_createInterpolatedFragmentAttribute(fragment, primitive, attribute,
...
void gpu_rasterizeTriangle(GPU const gpu, GPUPrimitive const *const primitive,
  ...
  for (size_t vertex = 0; vertex < VERTICES_PER_TRIANGLE; ++vertex)
    copy_Vec4_To_Vec2(triangleVertices + vertex,
  ...
  for (size_t y = yMinI; y < yMaxI; ++y) {
    ...
    for (size_t x = xMinI; x < xMaxI; ++x) {
      ...
      Vec3 barycentrics;
      gpu_computeScreenSpaceBarycentrics(&barycentrics, &pixelCoord,
                                         triangleVertices, triangleLines);
      gpu_createFragment(&fragmentShaderInput, primitive, &barycentrics,
                         &pixelCoord);
      ...
      gpu_perFragmentOperations(gpu, &fragmentShaderOutput,
...
void cpu_drawTriangles(GPU const gpu, size_t const nofVertices) {
  ...
  for (size_t base = 0; base + VERTICES_PER_TRIANGLE - 1 < nofVertices;
    ...
    // assembly primitive
    gpu_runPrimitiveAssembly(gpu, &primitive, VERTICES_PER_TRIANGLE, puller,
                             base, vertexShader);
    ...
    for (size_t c = 0; c < clippedTriangles.nofTriangles; ++c) {
      ...
      gpu_createSubPrimitive(&subPrimitive, &primitive,
